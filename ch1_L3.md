...
در هر دو نمایش، نام متغیرها با نشانه‌هایی دنبال می‌شوند که نوع هر متغیر را به شما می‌گویند: `<int>` نشان‌دهنده اعداد صحیح، `<dbl>` نشان‌دهنده اعداد حقیقی، `<chr>` برای کارکتر (یا رشته‌ها) و `<dttm>` نشان‌دهنده تاریخ-زمان است.
...
یک میانبر مفید برای ترکیب `|` و `==` وجود دارد: `%in%`. سطرهایی را انتخاب می‌کند که متغیر با یکی از مقادیر سمت راست برابر باشد:
```{r}
...
تابع `()arrange` ترتیب سطرها را بر اساس مقدار ستون‌ها تغییر می‌دهد. یک مجموعه داده و مجموعه‌ای از نام ستون‌ها (یا عبارت‌های پیچیده‌تر) برای مرتب‌سازی می‌گیرد. اگر بیش از یک نام ستون ارائه دهید، هر ستون اضافی برای شکستن گره‌ها در مقدارهای ستون‌های قبلی استفاده خواهد شد. برای مثال، کد زیر بر اساس زمان حرکت که در چهار ستون پخش شده‌اند، مجموعه داده را مرتب می‌کند. ابتدا زودترین سال‌ها، سپس در هر سال، زودترین ماه‌ها و ... را دریافت می‌کنیم.
...
`.` نشان می‌دهد که `before.` آرگومان تابع است و نه نام متغیر سومی که در حال ایجاد آن هستیم. همچنین می‌توانید از `after.` برای اضافه کردن بعد از یک متغیر استفاده کنید، و در هر دو آرگومان `before.` و `after.` می‌توانید از نام متغیر به جای موقعیت استفاده کنید. برای مثال، می‌توانیم متغیرهای جدید را بعد از `day` اضافه کنیم:
...
نمونه‌های ساده pipe را در بالا نشان داده‌ایم، اما قدرت واقعی و فراوانی کاربرد آن زمانی پدیدار می‌شود که خواهان ترکیب چندین تابع باشید. برای مثال، تصور کنید می‌خواهید سریع‌ترین پروازها به فرودگاه IAH هیوستون را پیدا کنید: باید `filter()`، `mutate()`، `select()`، و `arrange()` را ترکیب کنید:
...
اگرچه این pipeline چهار مرحله دارد، خواندن آن آسان است چون تابع‌ها در ابتدای هر خط می‌آیند: با داده `flights` شروع کنید، سپس filter، سپس mutate، سپس select و در آخر arrange.

اگر pipe را نداشتیم چه اتفاقی می‌افتاد؟ می‌توانستیم هر فراخوانی تابع را درون فراخوانی تابع قبلی (تودرتو) قرار دهیم:
...
برای افزودن pipe به کد خود، توصیه می‌کینم از کلیدهای میانبر  Ctrl/Cmd + Shift + M تعبیه‌شده در RStudio استفاده کنید. همان طور که در شکل زیر نشان داده شده است، 
![Alt text](images/rstudio-pipe-options.png)
 برای استفاده از `<|` به جای `%<%` نیاز دارید تا یک تغییر در تنظیمات RStudio انجام دهید (گزینه‌ی "+Use native pipe operator |>, require R4.1" را فعال کنید).

> بسته ‍‍magrittr
> 
> اگر مدت زمانی از بسته tidyverse استفاده کرده باشید، ممکن است با عملگر pipe `%<%` ارائه‌ شده توسط بسته **magrittr** آشنا باشید. بسته magrittr در هسته tidyverse گنجانده شده است، بنابراین می‌توانید هر زمان که tidyverse را بارگذاری کرده‌اید از `%<%` استفاده کنید:
> ```{r}
> library(tidyverse)
>
> mtcars %>% 
...
تاکنون درباره تابع‌هایی که با سطرها و ستون‌ها کار می‌کنند آشنا شده‌اید. وقتی توانایی کار با گروه‌ها را هم اضافه کنید، خواهید دید که بسته dplyr حتی قدرتمندتر هم می‌شود. در این بخش، روی مهم‌ترین تابع‌ها تمرکز خواهیم کرد: `group_by()`، `summarize()`، و خانواده تابع‌های slice.

### 1.5.3 تابع `()group_by`

از `()group_by` برای تقسیم مجموعه داده به گروه‌های معنادار برای تحلیل استفاده کنید:
...
تابع `()group_by` داده را تغییر نمی‌دهد اما اگر دقیق به خروجی نگاه کنید، متوجه خواهید شد که نشان می‌دهد مجموعه داده بر اساس ماه گروه‌بندی شده است (`Groups: month [12]`). این بدان معناست که عملیات بعدی حالا «بر اساس ماه» کار خواهند کرد. `()group_by` این ویژگی گروه‌بندی (که به عنوان رده شناخته می‌شود) را به مجموعه داده اضافه می‌کند که رفتار تابع‌های  اعمال‌شده بعدی روی داده را تغییر می‌دهد.

### 2.5.3 تابع `()summarize`

مهم‌ترین کار گروه‌بندی خلاصه‌سازی است که اگر برای محاسبه یک آماره خلاصه استفاده شود، مجموعه داده را به یک سطر واحد برای هر گروه کاهش می‌دهد. در بسته dplyr، این کار توسط `()summarize` انجام می‌شود، همانند مثال زیر که میانگین دیر حرکت کردن (تاخیر) پروازها را بر حسب ماه محاسبه می‌کند:
...
همانطور که در بالا می‌بینید، مشکلی پیش آمده است و همه نتایج `NA` (تلفظ می‌شود "N-A") هستند، که نماد R برای مقدار گمشده است. این رخ داده است چون برخی از پروازهای مشاهده‌شده در ستون تأخیر "delay"، داده گمشده‌‌ای داشتند و بنابراین وقتی میانگین شامل آن مقادیر را محاسبه کردیم، نتیجه `NA` گرفتیم. در درس‌های آینده در مورد مقادیر گمشده با جزئیات بیشتری سخن خواهیم گفت، اما فعلاً با تنظیم آرگومان `na.rm` روی `TRUE` به تابع `()mean` می‌گوییم همه مقادیر گمشده را نادیده بگیرد:
...
می‌توانید تعداد دلخواهی خلاصه در یک فراخوانی `()summarize` ایجاد کنید. در درس‌های آینده خلاصه‌های مختلف مفیدی را یاد خواهید گرفت، اما یک خلاصه بسیار مفید `()n` است که تعداد سطرها در هر گروه را برمی‌گرداند:
...
### 3.5.3 تابع‌های `_slice`

پنج تابع مفید وجود دارد که به وسیله‌ی آنها می‌توانید سطرهای خاص درون هر گروه را انتخاب کنید و بیرون بکشید:

- دستور `df |> slice_head(n = 1)` نخستین سطر از هر گروه را انتخاب می‌کند.
...
می‌توانید `n` را تغییر دهید تا بیش از یک سطر انتخاب کنید، یا به جای `= n`، می‌توانید از `prop = 0.1` استفاده کنید تا (مثلاً) 10 درصد سطرها در هر گروه را انتخاب کنید. برای مثال، کد زیر پروازهایی که بیشترین دیرکرد (تاخیر) ورود در هر مقصد دارند را پیدا می‌کند:
...
توجه کنید که 105 مقصد وجود دارد اما در اینجا 108 سطر نمایش داده شده است. چه اتفاقی افتاده است؟ توابع `()slice_min` و `()slice_max` مقادیر وابسته را حفظ می‌کنند، بنابراین `1 = n` به این معنی است که همه ردیف‌هایی که بالاترین مقدار را دارند به ما می‌دهد. اگر دقیقاً یک سطر در هر گروه می‌خواهید می‌توانید `with_ties = FALSE` تنظیم کنید. این شبیه محاسبه بیشترین مقدار دیرکرد با تابع `()summarize` است با این تفاوت که اینجا تمام سطر مربوطه را دارید در حالی که تابع `()summarize` تنها یک آماره خلاصه به عنوان خروجی می‌دهد. 
...
وقتی tibble گروه‌بندی شده با بیش از یک متغیر را خلاصه می‌کنید، هر خلاصه آخرین گروه را حذف می‌کند. با نگاه به گذشته، متوجه می‌شویم که این روش خوبی برای کارکرد این تابع نیست، اما تغییر آن بدون شکستن کد موجود دشوار است. برای این که مشخص شود چه رخ می‌دهد، بسته dplyr پیامی نمایش می‌دهد که به شما می‌گوید چگونه می‌توانید این رفتار را تغییر دهید:
...
همچنین ممکن است بخواهید گروه‌بندی را از مجموعه داده بدون استفاده از `()summarize` حذف کنید. می‌توانید این کار را با `()ungroup` انجام دهید.
...
حالا ببینیم وقتی یک مجموعه داده گروه‌بندی‌نشده را خلاصه می‌کنید چه اتفاقی می‌افتد.
...
یک سطر واحد در خروجی می‌بینید چون بسته dplyr همه سطرها در یک مجموعه داده گروه‌بندی‌نشده را متعلق به یک گروه در نظر می‌گیرد.

### 6.5.3 آرگومان `by.`
بسته dplyr 1.1.0 شامل یک دستور جدید و آزمایشی برای گروه‌بندی هر عملیات، یعنی آرگومان `by.` است. تابع‌های `()group_by` و `()ungroup` حذف نمی‌شوند، اما اکنون می‌توانید از آرگومان `by.` نیز برای گروه‌بندی درون یک عملیات واحد نیز استفاده کنید:
...
آرگومان `by.` با همه توابع کار می‌کند و این مزیت را دارد که نیازی به استفاده از آرگومان `groups.` برای عدم نمایش پیام گروه‌بندی یا `()ungroup` پس از اتمام کار ندارید.

ما در این فصل روی این آرگومان تمرکز نکردیم، زیرا هنگام نوشتن کتاب بسیار جدید بود. می‌خواستیم به آن اشاره کنیم چون فکر می‌کنیم پتانسیل زیادی دارد و احتمالاً بسیار محبوب خواهد شد. می‌توانید در پست وبلاگ
[dplyr 1.1.0](https://www.tidyverse.org/blog/2023/02/dplyr-1-1-0-per-operation-grouping/)
درباره آن بیشتر بدانید.

### 7.5.3 تمرینات
1. کدام شرکت هواپیمایی بدترین میانگین دیرکرد را دارد؟ چالش: آیا می‌توانید اثرات فرودگاه‌های بد را از اثرات شرکت‌های هواپیمایی بد جدا کنید؟ چرا/چرا نه؟ (راهنمایی: به دستور `flights |> group_by(carrier, dest) |> summarize(n())` نگاه کنید)
2. پروازهایی که بیشترین دیرکرد را هنگام پرواز از هر مقصد دارند، پیدا کنید.
3. دیرکردها در طول روز چگونه تغییر می‌کنند؟ پاسخ خود را با یک نمودار نشان دهید.
4. اگر به `()slice_min` و تابع‌های مرتبط یک n منفی بدهید، چه اتفاقی می‌افتد؟
5. بر اساس تابع‌های dplyr که تازه یاد گرفته‌اید، توضیح دهید که تابع `()count` چه کاری انجام می‌دهد. همین طور، شرح دهید که آرگومان `sort` در تابع `()count` چه چیزی انجام می‌دهد.
6. فرض کنید یک مجموعه داده کوچک به صورت زیر داریم:
```{r}
df <- tibble(
  x = 1:5,
  y = c("a", "b", "a", "a", "b"),
...
```
  الف. چیزی را که فکر می‌کنید خروجی دستور زیر باشد، بنویسید و سپس بررسی کنید که آیا درست حدس زده بودید، و همچنین کاری را که تابع `()group_by` انجام می‌دهد نیز شرح دهید.
 ```{r}
 df |>
   group_by(y)
 ```
  ب. چیزی را که فکر می‌کنید خروجی دستور زیر باشد، بنویسید و سپس بررسی کنید که آیا درست حدس زده بودید، و همچنین کاری را که تابع `()arrange` انجام می‌دهد نیز شرح دهید. همچنین، در مورد تفاوت آن با تابع `()group_by` در بخش (الف) شرح دهید.
 ```{r}
 df |>
   arrange(y)
 ```
  ج. چیزی را که فکر می‌کنید خروجی دستور زیر باشد، بنویسید و سپس بررسی کنید که آیا درست حدس زده بودید، و کاری که pipeline انجام می‌دهد را نیز شرح دهید.
 ```{r}
 df |>
   group_by(y) |>
   summarize(mean_x = mean(x))
 ```
  د. چیزی را که فکر می‌کنید خروجی دستور زیر باشد، بنویسید و سپس بررسی کنید که آیا درست حدس زده بودید، و کاری که pipeline انجام می‌دهد را نیز شرح دهید. سپس، در مورد آنچه در پیام آمده است، نظر دهید.
 ```{r}
 df |>
   group_by(y, z) |>
   summarize(mean_x = mean(x))
 ```
  ه. چیزی را که فکر می‌کنید خروجی دستور زیر باشد، بنویسید و سپس بررسی کنید که آیا درست حدس زده بودید، و کاری که pipeline انجام می‌دهد را نیز شرح دهید. خروجی چه تفاوتی با خروجی بخش (د) دارد؟
 ```{r}
 df |>
   group_by(y, z) |>
   summarize(mean_x = mean(x), .groups = "drop")
 ```
  و. چیزی را که فکر می‌کنید خروجی دستور زیر باشد، بنویسید و سپس بررسی کنید که آیا درست حدس زده بودید، و کاری که pipeline انجام می‌دهد را نیز شرح دهید. خروجی‌های این دو pipeline چه تفاوتی با هم دارند؟
 ```{r}
df |>
  group_by(y, z) |>
  summarize(mean_x = mean(x))
...
## 6.3 مطالعه موردی: انباشتگی‌ها و اندازه نمونه
هر وقت تجمیعی انجام می‌دهید، همیشه ایده خوبی است که شمارش (`()n`) را نیز در کنار آن داشته باشید. بدین ترتیب، می‌توانید اطمینان حاصل کنید که نتیجه‌گیری بر اساس تعداد بسیار کم داده نیست. این را با برخی داده‌های بیسبال از بسته **Lahman** نشان خواهیم داد. به طور خاص، نسبت دفعاتی که بازیکن با توپ ضربه می‌خورد را (`H`) در مقابل تعداد دفعاتی که سعی می‌کند توپ را بازی کند (`AB`) مقایسه خواهیم کرد:
...
```
![Alt text](images/unnamed-chunk-56-1.png)


به الگوی مفید ترکیب ggplot2 و dplyr توجه کنید. فقط باید یادتان باشد که `<|` برای پردازش مجموعه داده را به `+` برای اضافه کردن لایه‌ها به نمودارتان تغییر دهید.

این موضوع همچنین معانی مهمی برای رتبه‌بندی دارد. اگر به طور ساده بر اساس `desc(performance)` مرتب‌سازی کنید، افرادی که بهترین میانگین ضربه زدن را دارند به وضوح کسانی هستند که دفعه‌های بسیار کمی سعی کرده‌اند توپ را در بازی قرار دهند و اتفاقاً ضربه‌ای هم دریافت کرده‌اند، آنها لزوماً ماهرترین بازیکنان نیستند:
```{r}
batters |> 
...
#> # ℹ 20,724 more rows
```
می‌توانید توضیح خوبی در مورد این مشکل و روش برطرف کردن آن در [http://varianceexplained.org/r/empirical_bayes_baseball/](http://varianceexplained.org/r/empirical_bayes_baseball/) و [https://www.evanmiller.org/how-not-to-sort-by-average-rating.html](https://www.evanmiller.org/how-not-to-sort-by-average-rating.html) پیدا کنید.

## خلاصه

در این فصل، ابزارهایی که بسته‌ی dplyr برای کار با ‌مجموعه‌ داده‌ها فراهم می‌کند را یاد گرفته‌اید. ابزارها تقریباً در سه دسته گروه‌بندی شده‌اند: آنهایی که سطرها را دستکاری می‌کنند (مثل `()filter` و `()arrange`)، آنهایی که ستون‌ها را دستکاری می‌کنند (مثل `()select` و `()mutate`) و آنهایی که گروه‌ها را دستکاری می‌کنند (مثل `()group_by` و `()summarize`). در این فصل، بر روی ابزارهایی برای کار با کل مجموعه داده تمرکز کرده‌ایم، اما هنوز چیز زیادی درباره آنچه می‌توانید با متغیرهای فردی انجام دهید یاد نگرفته‌اید. در بخش «تبدیل» کتاب، که هر فصل ابزارهایی برای نوع خاصی از متغیر فراهم می‌کند، به آن بازخواهیم گشت.

در درس بعد، به موضوع گردش کار بازخواهیم پرداخت تا درباره اهمیت سبک کد و سازماندهی خوب کد برای آسان کردن خواندن و درک آن برای شما و دیگران سخن بگوئیم.
